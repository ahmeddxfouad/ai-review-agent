...
- ✅ **migration files exist** — Glob pattern matched: migrations/**/*
  ...
  - Total matches: 1
- ✅ **Migration files reference required tables** — Found files matching expected keywords: ['migrations\\001-create-users.sql', 'users.spec.ts']
  - Review method: Static inspection; runtime not run
...
> **Passing ✅** — The database design includes evidence of the required relational tables.

## General Feedback

Your migrations show evidence of a relational schema that supports the project requirements. The required table groups are represented, and the database structure can be reviewed against the documented API data shapes.

## Requirement Status

- **Required tables:** ✔ Migration evidence exists for the project tables.
- **Relationships:** ✔ The schema includes evidence for order/product relationships.
- **Reviewability:** ✔ The migrations give reviewers a concrete source for table and column verification.

## Recommendations

- Confirm the actual database after migration with `psql` commands such as `\dt` and `\d table_name`.
- Avoid reserved or confusing names like `count`; use `quantity` for order/product quantity.
...
- ❌ **Route or handler files exist** — No files matched expected keywords: ['handler', 'handlers', 'route', 'routes']
  ...
  - Matches: none
  - Total matches: 0
- ✅ **Files reference required API resources** — Found files matching expected keywords: ['migrations\\001-create-users.sql', 'users.spec.ts']
  - Review method: Static inspection; runtime not run
  - Matches: `migrations\001-create-users.sql`, `users.spec.ts`
  - Total matches: 2
...
- ❌ **Model folder/files exist** — No files matched expected keywords: ['models', 'model', 'store']
  ...
  - Matches: none
  - Total matches: 0
- ✅ **Model files reference required tables** — Found files matching expected keywords: ['migrations\\001-create-users.sql', 'users.spec.ts']
  - Review method: Static inspection; runtime not run
  - Matches: `migrations\001-create-users.sql`, `users.spec.ts`
  - Total matches: 2
...
- ✅ **Spec files exist for model/database action tests** — Glob pattern matched: **/*.spec.*
  ...
  - Matches: `users.spec.ts`
  - Total matches: 1
- ✅ **Tests or source files reference required database models** — Found files matching expected keywords: ['migrations\\001-create-users.sql', 'users.spec.ts']
  - Review method: Static inspection; runtime not run
  - Matches: `migrations\001-create-users.sql`, `users.spec.ts`
  - Total matches: 2
...
> **Passing ✅** — Database action tests are present for review.

## General Feedback

Your project includes test infrastructure and files that can be used to verify database model actions. Reviewers should run the tests and compare coverage against every required model method.

## Requirement Status

- **Database action tests exist:** ✔ Spec files are present.
- **Tests can be run:** ✔ A `test` script is available.
- **Required models are represented:** ✔ Project files reference users, products, and orders.

## Recommendations

- Add one test per model method, including `index`, `show`, `create`, `update`, and `delete` where required.
- Use a test database and clean up created records between tests.
- Include useful edge cases, such as missing IDs or invalid credentials.
